...
class ProviderApiController extends Controller
{
    ...
    public function services_(Request $request, $service_id)
    {
        ...
        $providerServices    =  ProviderServices::where('service_id', $service_id)
            ...
            ->with('provider:id,number_phone,username,active,verified,country_id', 'rating', 'quickOffer', 'offers', 'serviceType');
    ...
    public function serviceUpdate(Request $request, $provider_service_id)
    {
        ...
        $providerService    =    ProviderServices::where('id', $provider_service_id)->firstOrFail();
        $in_update          =
    ...
    {
        $providerService                  =   ProviderServices::where('id', $provider_service_id)->with('ratings', 'offers', 'provider', 'serviceType')->first();
        ...
        $chatList = $chatList->map(function ($item) {
            ...
            return  [
                ...
                'user'         => [
                    ...
                    "name"          => $item->user->role === 'chat_review' ? 'دكتورتك' : $item->user->username,
                    "image"         => $image,
                    "type"          => $item->user->role,
    ...
    public function orderStatistics(Request $request)
    {
        ...
        $ordersCount              =  Order::where('provider_id', $provider_id)->statistics()->first();

        $ordersCount->revenue     =  Order::where('status', 'COMPLETED')->where('provider_id', $provider_id)->has('provider_service_not_online')->sum('price');

        $ordersCount->commission  =  Order::where('status', 'COMPLETED')->where('provider_id', $provider_id)->has('provider_service_not_online')->sum('commission') ?? 0;
        ...
        if ($request->id != Null) {
            $provider_info        =  User::where('id', $provider_id)->select('id', 'country_id')->first();
        ...
        $ordersCount->transaction    =  collect($trans)->map(function ($item) {
            return  [
                'id'          => $item->id,
    ...
    public function change_services_status(Request $request)
    {
        ...
        $providerServices            =   ProviderServices::where('id', $fields['id'])->with('provider:id,device_token')->first();
